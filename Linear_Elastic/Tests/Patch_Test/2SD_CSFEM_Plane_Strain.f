...
C    DECLARATION OF THE VARIABLES
      ...
      DOUBLE PRECISION, DIMENSION(3,1) :: STRAIN_1SD
      DOUBLE PRECISION, DIMENSION(3,1) :: STRAIN_2SD
      DOUBLE PRECISION, DIMENSION(3,1) :: STRESS_1SD
      DOUBLE PRECISION, DIMENSION(3,1) :: STRESS_2SD
      DOUBLE PRECISION, DIMENSION(2,7) :: OUTPUT_MATRIX
...
C   FOR 1SD 
      DO I = 1,3
        DO J = 1,8 
            STRAIN_1SD(I,1) = STRAIN_1SD(I,1) + B(I,J)*U(J)
        END DO 
      END DO 
      DO I = 1,3
        DO J = 1,8 
            STRAIN_2SD(I,1) = STRAIN_2SD(I,1) + B(I+3,J)*U(J)
        END DO 
      END DO
      DO I = 1,3
        DO J = 1,3
            STRESS_1SD(I,1) = STRESS_1SD(I,1) + CMATRIX(I,J)*
     1      STRAIN_1SD(I,1)
        END DO 
      END DO
      DO I = 1,3
        DO J = 1,3
            STRESS_2SD(I,1) = STRESS_2SD(I,1) + CMATRIX(I,J)*
     1      STRAIN_2SD(I,1)
        END DO 
      END DO
C   OUTPUT MATRIX 
      OUTPUT_MATRIX(1,1) = STRESS_1SD(1,1)
      OUTPUT_MATRIX(1,2) = STRESS_1SD(2,1)
      OUTPUT_MATRIX(1,3) = STRESS_1SD(3,1)
      OUTPUT_MATRIX(1,4) = STRAIN_1SD(1,1)
      OUTPUT_MATRIX(1,5) = STRAIN_1SD(2,1)
      OUTPUT_MATRIX(1,6) = STRAIN_1SD(3,1)
      OUTPUT_MATRIX(1,7) = AREA_CELL(1)
      OUTPUT_MATRIX(2,1) = STRESS_2SD(1,1)
      OUTPUT_MATRIX(2,2) = STRESS_2SD(2,1)
      OUTPUT_MATRIX(2,3) = STRESS_2SD(3,1)
      OUTPUT_MATRIX(2,4) = STRAIN_2SD(1,1)
      OUTPUT_MATRIX(2,5) = STRAIN_2SD(2,1)
      OUTPUT_MATRIX(2,6) = STRAIN_2SD(3,1)
      OUTPUT_MATRIX(2,7) = AREA_CELL(2)
      PRINT *, JELEM
      PRINT *, OUTPUT_MATRIX
      OPEN(111, file='C:\Temp\Linear_Elasticity\Second_CSFEM\Strain\Output_Matrix.csv',status='unknown')
      WRITE(111,*) 'JELEM',JELEM
      DO I = 1,2
        DO J = 1,7
            WRITE(111,*) OUTPUT_MATRIX(I,J)
        END DO 
      END DO
C   Saving SVARS
      SVARS(1) = AREA_CELL(1)
